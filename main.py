...
def main():
    ...
    raw_mtr_lines_and_stations = fetch_or_load_from_cache("mtr_lines_and_stations", mtr_rails_client.fetch_mtr_lines_and_stations, args.force_ingest, silent=args.silent)
    ...
    citybus_stops_data = fetch_or_load_from_cache("citybus_stops_and_sequences", citybus_client.fetch_all_stops_threaded, args.force_ingest, raw_citybus_routes, silent=args.silent)
    raw_citybus_stop_id, raw_citybus_route_sequences = citybus_stops_data if citybus_stops_data else ([], [])
    raw_citybus_stop_details = fetch_or_load_from_cache("citybus_stop_details", citybus_client.fetch_all_stop_details_threaded, args.force_ingest, raw_citybus_stop_id, silent=args.silent)

    # NLB
    raw_nlb_routes = fetch_or_load_from_cache("nlb_routes", nlb_client.fetch_all_routes, args.force_ingest, silent=args.silent)
    nlb_stops_data = fetch_or_load_from_cache("nlb_stops_and_route_stops", nlb_client.fetch_all_stops_and_route_stops_threaded, args.force_ingest, raw_nlb_routes, silent=args.silent)
    ...
    # OSM
    raw_osm_routes = fetch_or_load_from_cache("osm_routes", osm_parser.fetch_osm_routes, args.force_ingest, silent=args.silent)
    if not args.silent:
